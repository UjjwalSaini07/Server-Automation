...
API_URL = os.getenv("AFFILIATE_HEALTH_URL", "https://affiliate-postback-engine.onrender.com/health")
MONGO_URI = os.getenv("MONGODB_URI", "mongodb://localhost:27017")
DB_NAME = "Affiliate-ServerAutomation"
...
def ping_health_api():
    if not is_within_allowed_hours():
        print(f"[{datetime.now(timezone.utc)}] Skipping health check (outside allowed IST hours).")
        return
...
def start():
    schedule.every(15).minutes.do(ping_health_api)
    print(f"Server : Affiliate health check service started (All Week - IST 9:00 → 2:00).")
